...
- Прикладные правила превосходства ставок (шаг не менее 100 ₽), стартовой цены продажи и переходов статусов находятся в сервисном слое.
- `Lot.current_price` — вычисляемое свойство модели (не хранится в БД): цена продажи, если лот продан; иначе максимальная ставка; иначе стартовая цена. Отдаётся в `LotRead` как `current_price`.
